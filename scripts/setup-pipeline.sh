...
ANALYZER_TIMING_COMMAND="\"$ANALYZER_VENV_PYTHON ./scripts/beat-this-backend.py --input {input} --output {output}\""
ANALYZER_EVENT_COMMAND="\"$ANALYZER_VENV_PYTHON ./scripts/backend-analyzer.py --input {input} --output {output}\""
ANALYZER_PRIMARY_COMMAND="\"$ANALYZER_VENV_PYTHON ./scripts/hybrid-drum-events-backend.py --input {input} --output {output}\""
ANALYZER_FALLBACK_COMMAND="$ANALYZER_EVENT_COMMAND"
...
if [[ "$BOOTSTRAP_ANALYZER" == true ]]; then
  ...
  replace_or_append_env "$ENV_FILE" "PIPELINE_ANALYZER_TIMING_BACKEND_COMMAND" "$ANALYZER_TIMING_COMMAND"
  replace_or_append_env "$ENV_FILE" "PIPELINE_ANALYZER_EVENT_BACKEND_COMMAND" "$ANALYZER_EVENT_COMMAND"
  replace_or_append_env "$ENV_FILE" "PIPELINE_ANALYZER_EVENT_POLICY" "optional"
...
printf '[analyzer-check] PIPELINE_ANALYZER_TIMING_BACKEND_COMMAND=%s\n' "\${PIPELINE_ANALYZER_TIMING_BACKEND_COMMAND:-}"
printf '[analyzer-check] PIPELINE_ANALYZER_EVENT_BACKEND_COMMAND=%s\n' "\${PIPELINE_ANALYZER_EVENT_BACKEND_COMMAND:-}"
printf '[analyzer-check] PIPELINE_ANALYZER_EVENT_POLICY=%s\n' "\${PIPELINE_ANALYZER_EVENT_POLICY:-}"
